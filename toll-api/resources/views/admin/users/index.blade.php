...
@section('content')
    ...
    <div class="grid grid-cols-1 gap-6 mb-8 sm:grid-cols-2 lg:grid-cols-4">
        <div class="p-4 bg-white rounded-xl shadow-sm user-card border border-gray-100">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-lg font-semibold text-gray-800">{{ $users->total() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-4 bg-white rounded-xl shadow-sm user-card border border-gray-100">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-lg font-semibold text-gray-800">Rp {{ number_format($users->sum('saldo'), 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-4 bg-white rounded-xl shadow-sm user-card border border-gray-100">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-lg font-semibold text-gray-800">{{ $users->where('created_at', '>=', now()->subDays(7))->count() }}</p>
        ...
        <div class="p-4 bg-white rounded-xl shadow-sm user-card border border-gray-100">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-500">Rata-rata Saldo</p>
                    <p class="text-lg font-semibold text-gray-800">Rp {{ number_format($users->avg('saldo'), 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
    </div>
            ...
            <form action="{{ route('admin.users.index') }}" method="GET">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                    <div class="lg:col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="search" id="search" value="{{ request('search') }}" 
                                placeholder="Cari nama, email, atau plat nomor" 
                    ...
                    <div>
                        ...
                        <select name="vehicle_type" id="vehicle_type" class="form-input block w-full py-3 px-4 rounded-xl">
                    ...
                    <div>
                        ...
                        <select name="sort_by" id="sort_by" class="form-input block w-full py-3 px-4 rounded-xl">
                            <option value="created_at" {{ request('sort_by') == 'created_at' ? 'selected' : '' }}>Tanggal Daftar</option>
                            <option value="name" {{ request('sort_by') == 'name' ? 'selected' : '' }}>Nama</option>
                            <option value="saldo" {{ request('sort_by') == 'saldo' ? 'selected' : '' }}>Saldo</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="sort_order" id="sort_order" class="form-input block w-full py-3 px-4 rounded-xl">
                ...
                <div class="filter-actions flex items-center justify-between">
                    <div>
                        @if(request('search') || request('vehicle_type') || request('sort_by') || request('sort_order'))
                            <a href="{{ route('admin.users.index') }}" class="btn-reset inline-flex items-center">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($users as $user)
                        <tr class="table-row">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center">
                                        <span class="font-medium text-blue-600">{{ substr($user->name, 0, 1) }}</span>
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="badge badge-blue">{{ $user->plat_nomor }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($user->kelompok_kendaraan)
                                    <span class="badge {{ $user->kelompok_kendaraan == 'Mobil' ? 'badge-green' : ($user->kelompok_kendaraan == 'Bus' ? 'badge-orange' : 'badge-blue') }}">
                                        {{ $user->kelompok_kendaraan }}
                                    </span>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium {{ $user->saldo < 0 ? 'text-red-600' : 'text-green-600' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-3">
                                    <a href="{{ route('admin.users.show', $user->id) }}" class="text-blue-600 hover:text-blue-900 action-button" title="Lihat Detail">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
                                    </a>
                                    <a href="{{ route('admin.users.edit', $user->id) }}" class="text-indigo-600 hover:text-indigo-900 action-button" title="Edit">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-400 text-sm mt-1">Tidak ada pengguna yang ditemukan dengan filter yang diterapkan</p>
                                    
                                    @if(request('search') || request('vehicle_type') || request('sort_by') || request('sort_order'))
                                        <a href="{{ route('admin.users.index') }}" class="mt-4 inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                                            </svg>
                                            Reset Filter
